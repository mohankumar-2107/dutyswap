...
export default function EmployeeDashboard() {
  ...
  // ✅ Inline complete task mutation — properly invalidates the right query key
  const completeTaskMutation = useMutation({
    ...
    onSuccess: () => {
      // Invalidate with no filter — catches all task query key variations
      queryClient.invalidateQueries();
      toast({ title: "✅ Task Completed!", description: "Great work! Task marked as done." });
    },
    onError: () => {
  ...
  // Invalidate all queries on mount
  useEffect(() => {
    if (user?.id) {
  ...
  // Poll help requests every 3s to detect acceptance
  ...
  // Notify requester when request is accepted — redirect to Help Inbox
  useEffect(() => {
    if (!user?.id || !Array.isArray(helpRequests) || helpRequests.length === 0) return;
    const acceptedRequest = helpRequests.find(
      (r: any) => r.requesterId === user.id && r.status === "accepted"
    );
    if (acceptedRequest) {
      toast({
        title: "🎉 Request Accepted!",
        description: `${acceptedRequest.helper?.name || "Your peer"} accepted your request. Go to Help Inbox to chat!`
      });
    }
  }, [helpRequests.length]);

  // Low-stress eligible helpers (excluding self)
  const { data: eligibleHelpers } = useQuery<any[]>({
    queryKey: ["/api/employees"],
    select: (emps) =>
      emps.filter((e: any) => e.role === "employee" && e.id !== user?.id && (e.currentStress || 0) <= 3)
  });

  // Send peer assistance request
  ...
  // Incoming requests where this user is the helper
  const incomingRequests = Array.isArray(helpRequests)
    ? helpRequests.filter((r: any) => r.helperId === user.id && r.status === "pending")
    : [];

  // Has any accepted chat (either side) — show "Open Help Inbox" button
  const hasActiveChat =
    Array.isArray(helpRequests) &&
    helpRequests.some(
      (r: any) =>
        (r.requesterId === user.id || r.helperId === user.id) && r.status === "accepted"
    );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700">
                You haven't updated your stress levels today. Please take a moment to check in.
              </p>
            </div>
          </div>
          <Link href="/wellness">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black font-display tracking-tight text-gray-900">
              Hello, {user.name.split(" ")[0]}!
            </h1>
          </div>
          <p className="text-gray-500 font-medium ml-1">
            Your personal wellness & productivity mission control.
          </p>
        </motion.div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card className="glass-card border-none shadow-xl rounded-3xl p-6">
            ...
            <CardContent className="px-0 space-y-4">
              ...
              {/* Eligible helpers list */}
              {eligibleHelpers && eligibleHelpers.length > 0 ? (
                <div className="space-y-2">
                  {eligibleHelpers.map((helper: any) => (
                    <div
                      key={helper.id}
                      className="flex items-center justify-between p-3 bg-yellow-50/50 rounded-2xl border border-yellow-100"
                    >
                      <div>
                ...
                <p className="text-xs text-muted-foreground italic">
                  No eligible helpers available right now.
                </p>
              )}

              {/* Incoming requests (this user is the helper) */}
              {incomingRequests.length > 0 && (
                <div className="mt-6 pt-6 border-t border-gray-100 space-y-3">
                  ...
                  {incomingRequests.map((req: any) => (
                    <div
                      key={req.id}
                      className="bg-yellow-400/10 p-4 rounded-2xl border border-yellow-200"
                    >
                      <p className="text-sm font-bold mb-3">
                        {req.requester?.name} has requested your assistance.
                      </p>
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() =>
                            respondHelpMutation.mutate(
                              { id: req.id, status: "accepted" },
                              {
                                onSuccess: () => {
                                  toast({
                                    title: "✅ Accepted!",
                                    description: `Go to Help Inbox to chat with ${req.requester?.name}.`
                                  });
                                }
                              }
                            )
                        ...
                        <Button
                          ...
                          className="rounded-xl flex-1 font-bold"
                          onClick={() =>
                            respondHelpMutation.mutate({ id: req.id, status: "rejected" })
                          }
                          disabled={respondHelpMutation.isPending}
                        >
                          Decline
              ...
              {/* Open Help Inbox button when a chat is active */}
              {hasActiveChat && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <Link href="/help-inbox">
                    <Button className="w-full bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold">
                      💬 Open Help Inbox to Chat
                    </Button>
                  </Link>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="bg-gradient-to-br from-yellow-400 to-orange-500 border-none shadow-2xl rounded-[2.5rem] text-white overflow-hidden relative group">
              ...
              <CardContent className="space-y-4 pt-2">
                ...
                <div className="flex gap-2">
                  <span className="text-[10px] font-black uppercase px-2 py-1 bg-white/30 rounded-full tracking-widest">
                    Self-Care
                  </span>
                  <span className="text-[10px] font-black uppercase px-2 py-1 bg-white/30 rounded-full tracking-widest">
                    Wellness
                  ...
                  <AnimatePresence>
                    {tasks?.map((task: any) => (
                      <motion.div
                        ...
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className={`p-6 flex items-center gap-6 group hover:bg-yellow-50/30 transition-all ${
                          task.status === "Completed" ? "bg-gray-50/50 opacity-60" : ""
                        }`}
                      >
                        <Button
                          variant="ghost"
                          ...
                          className={`w-12 h-12 rounded-2xl shadow-sm transition-all ${
                            task.status === "Completed"
                              ...
                              : "bg-white border-2 border-gray-100 text-gray-300 hover:border-yellow-400 hover:text-yellow-500"
                          }`}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span
                              className={`text-[10px] px-3 py-1 rounded-full font-black tracking-widest uppercase border ${
                                task.priority === "High"
